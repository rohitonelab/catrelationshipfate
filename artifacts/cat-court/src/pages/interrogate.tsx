...
export default function InterrogatePage() {
  ...
  const currentQ = PARTNER_A_QUESTIONS[currentQIndex];
  ...
  const handleAnswer = (optionIndex: number) => {
    ...
    setCatState('typing'); // writing in notebook
    
    setTimeout(() => {
      if (currentQIndex < PARTNER_A_QUESTIONS.length - 1) {
        ...
        if (sessionIdRef.current) {
          submitAnswers.mutate({
            ...
            onSuccess: () => {
              setLocation(`/waiting/${sessionIdRef.current}`);
            }
          });
        }
      }
    }, 1500); // Give time for notebook animation and fade
  };
  ...
  const progress = (currentQIndex / PARTNER_A_QUESTIONS.length) * 100;

  return (
    <SceneTransition location="/interrogate">
